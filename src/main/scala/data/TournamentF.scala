...
case class TournamentF[A, B](name: String,
                             startDate: LocalDate,
                             endDate: LocalDate,
                             region: Option[A],
                             url: Uri,
...
object TournamentF {
  implicit def tournementBiFunctor: Bitraverse[TournamentF] = new Bitraverse[TournamentF] {
    ...
    def bifoldRight[A, B, C](fab: TournamentF[A, B], c: Eval[C])(f: (A, Eval[C]) => Eval[C],
    ...
    def bitraverse[G[_]: Applicative, A, B, C, D](fab: TournamentF[A, B])(f: A => G[C],
                                                                          g: B => G[D]): G[TournamentF[C, D]] = {
