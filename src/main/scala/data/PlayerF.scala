...
case class PlayerF[A, B, C](team: Option[A],
                            region: B,
                            nickname: String,
                            realname: String,
                            country: String,
...
object PlayerF {
  ...
  def teamPlayerHoleFunctor[B, C]: Traverse[({ type l[a] = PlayerF[a, B, C] })#l] =
    new Traverse[({ type l[a] = PlayerF[a, B, C] })#l] {
      ...
      def foldRight[A, BB](fa: PlayerF[A, B, C], lb: Eval[BB])(f: (A, Eval[BB]) => Eval[BB]): Eval[BB] =
        fa.team.foldRight(lb)(f)
  ...
  def regionPlayerHoleFunctor[A, C]: Traverse[({ type l[a] = PlayerF[A, a, C] })#l] =
    new Traverse[({ type l[a] = PlayerF[A, a, C] })#l] {
      ...
      def foldRight[B, BB](fa: PlayerF[A, B, C], lb: Eval[BB])(f: (B, Eval[BB]) => Eval[BB]): Eval[BB] =
        f(fa.region, lb)
      def traverse[G[_]: Applicative, B, BB](fa: PlayerF[A, B, C])(f: B => G[BB]): G[PlayerF[A, BB, C]] =
        f(fa.region).map(b => fa.copy(region = b))
    }

  def rolePlayerHoleFunctor[A, B]: Traverse[({ type l[a] = PlayerF[A, B, a] })#l] =
    new Traverse[({ type l[a] = PlayerF[A, B, a] })#l] {
