...
object APIResultF {
  implicit def apiResultFunctor: Bitraverse[APIResultF] = new Bitraverse[APIResultF] {
    ...
    def bifoldRight[A, B, C](fab: APIResultF[A, B], c: Eval[C])(f: (A, Eval[C]) => Eval[C], g: (B, Eval[C]) => Eval[C]): Eval[C] = {
      val cc = fab.results.foldRight(c)(f)
      fab.next.foldRight(cc)(g)
    }

    def bitraverse[G[_]: Applicative, A, B, C, D](fab: APIResultF[A, B])(f: A => G[C], g: B => G[D]): G[APIResultF[C, D]] = {
