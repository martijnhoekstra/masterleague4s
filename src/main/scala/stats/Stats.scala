...
case class HeroStats(picks: Int, wins: Int, bans: Int, firstpicks: Int, firstbans: Int, uncontested: Int) {
  ...
  def winLines: List[String] = if (picks == 0) List()
  ...
  def contestLines: List[String] = if (contested == 0) List(s"Uncontested so far for $uncontested matches")
  else List(s"$picks picks and $bans bans in $total matches (${pctOrNa(contestedRate)} contested)") :+ {
    if (firstpicks == 0 && firstbans == 0) "none of which as first pick or ban"
    else s"with $firstpicks first picks (${pctOrNa(firstPickRate)}) and $firstbans first bans (${pctOrNa(firstBanRate)})"
  }
...
object Stats {
  ...
  def forHeroInTourny(heroId: Long, tournyId: Long, matches: List[MatchF[_, Long, _, _, _, DraftF[Long, Long, PickF[Long, _]]]]): HeroStats = {
    val matchesInTourny = matches.filter(m => m.tournament == tournyId)
    heroStats(heroId, matchesInTourny)
  }

  def forHeroInPatch(heroId: Long, patchId: Long, matches: List[MatchF[Long, _, _, _, _, DraftF[Long, Long, PickF[Long, _]]]]): HeroStats = {
  ...
  def heroStats(heroId: Long, matches: List[MatchF[_, _, _, _, _, DraftF[Long, Long, PickF[Long, _]]]]): HeroStats = {
    matches.foldLeft(HeroStats.empty)((stats, m) => {
      ...
      val isfirstpick = firstdraft.picks.headOption.exists(p => p.hero == heroId) || seconddraft.picks.take(2).exists(p => p.hero == heroId)
