...
object Endpoints {
  ...
  val players =
    tag[IdPlayer][Uri](Uri.parse("https://api.masterleague.net/players/?format=json&page_size=100").require)
  val tournaments =
    tag[IdTournament][Uri](Uri.parse("https://api.masterleague.net/tournaments/?format=json&page_size=100").require)
